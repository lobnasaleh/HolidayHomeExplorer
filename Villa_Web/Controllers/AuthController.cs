        ...
        public async Task< IActionResult> Login( LoginRequestDTO obj)
        {
          APIResponse response=  await _authService.LoginAsync<APIResponse>(obj);
            if (response!=null && response.IsSuccess)
            {
                ...
                var jwt=handler.ReadJwtToken(model.Token);//te2ra el token ely reg3t men al api 3shan t extract menha 
                ...
                identity.AddClaim(new Claim(ClaimTypes.Role, jwt.Claims.FirstOrDefault(u=>u.Type=="role").Value));//claim name
